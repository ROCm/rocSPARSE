...
void testing_coo2csr_bad_arg(void)
{
    ...
    // Testing for (coo_row_ind == nullptr)
    {
        ...
        status = rocsparse_coo2csr(handle, coo_row_ind_null, nnz, m,
                                   csr_row_ptr, rocsparse_index_base_zero);
    ...
    // Testing for (csr_row_ptr == nullptr)
    {
        ...
        status = rocsparse_coo2csr(handle, coo_row_ind, nnz, m,
                                   csr_row_ptr_null, rocsparse_index_base_zero);
    ...
    // Testing for (handle == nullptr)
    {
        ...
        status = rocsparse_coo2csr(handle_null, coo_row_ind, nnz, m,
                                   csr_row_ptr, rocsparse_index_base_zero);
...
rocsparse_status testing_coo2csr(Arguments argus)
{
    ...
    auto dcoo_row_ind_managed =
        rocsparse_unique_ptr{device_malloc(sizeof(rocsparse_int)*nnz),
                             device_free};
    auto dcsr_row_ptr_managed =
        rocsparse_unique_ptr{device_malloc(sizeof(rocsparse_int)*(m+1)),
                             device_free};
    ...
    CHECK_HIP_ERROR(hipMemcpy(dcoo_row_ind, hcoo_row_ind.data(),
                              sizeof(rocsparse_int)*nnz, hipMemcpyHostToDevice));

    if (argus.unit_check)
    {
        CHECK_ROCSPARSE_ERROR(rocsparse_coo2csr(handle, dcoo_row_ind, nnz,
                                                m, dcsr_row_ptr, idx_base));

        // Copy output from device to host
        CHECK_HIP_ERROR(hipMemcpy(hcsr_row_ptr.data(), dcsr_row_ptr,
                                  sizeof(rocsparse_int)*(m+1), hipMemcpyDeviceToHost));
