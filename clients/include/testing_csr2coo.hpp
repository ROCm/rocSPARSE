...
void testing_csr2coo_bad_arg(void)
{
    ...
    // Testing for (csr_row_ptr == nullptr)
    {
        ...
        status = rocsparse_csr2coo(handle, csr_row_ptr_null, nnz, m,
                                   coo_row_ind, rocsparse_index_base_zero);
    ...
    // Testing for (coo_row_ind == nullptr)
    {
        ...
        status = rocsparse_csr2coo(handle, csr_row_ptr, nnz, m,
                                   coo_row_ind_null, rocsparse_index_base_zero);
    ...
    // Testing for (handle == nullptr)
    {
        ...
        status = rocsparse_csr2coo(handle_null, csr_row_ptr, nnz, m,
                                   coo_row_ind, rocsparse_index_base_zero);
...
rocsparse_status testing_csr2coo(Arguments argus)
{
    ...
    CHECK_HIP_ERROR(hipMemcpy(dcsr_row_ptr, hcsr_row_ptr.data(),
                              sizeof(rocsparse_int)*(m+1), hipMemcpyHostToDevice));

    if (argus.unit_check)
    {
        CHECK_ROCSPARSE_ERROR(rocsparse_csr2coo(handle, dcsr_row_ptr, nnz,
                                                m, dcoo_row_ind, idx_base));

        // Copy output from device to host
        CHECK_HIP_ERROR(hipMemcpy(hcoo_row_ind.data(), dcoo_row_ind,
                                  sizeof(rocsparse_int)*nnz, hipMemcpyDeviceToHost));
