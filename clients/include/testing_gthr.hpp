...
void testing_gthr_bad_arg(void)
{
    ...
    // testing for(nullptr == dx_ind)
    {
        ...
        status = rocsparse_gthr(handle, nnz, dy, dx_val, dx_ind_null, idx_base);
    ...
    // testing for(nullptr == dx_val)
    {
        ...
        status = rocsparse_gthr(handle, nnz, dy, dx_val_null, dx_ind, idx_base);
    ...
    // testing for(nullptr == dy)
    {
        ...
        status = rocsparse_gthr(handle, nnz, dy_null, dx_val, dx_ind, idx_base);
    ...
    // testing for(nullptr == handle)
    {
        ...
        status = rocsparse_gthr(handle_null, nnz, dy, dx_val, dx_ind, idx_base);
        verify_rocsparse_status_invalid_handle(status);
    }
}

template <typename T>
rocsparse_status testing_gthr(Arguments argus)
{
    ...
    if(nnz <= 0)
    {
        ...
        status = rocsparse_gthr(handle, nnz, dy, dx_val, dx_ind, idx_base);
    ...
    if(argus.unit_check)
    {
        ...
        CHECK_ROCSPARSE_ERROR(rocsparse_set_pointer_mode(handle, rocsparse_pointer_mode_host));
        CHECK_ROCSPARSE_ERROR(rocsparse_gthr(handle, nnz, dy, dx_val, dx_ind, idx_base));
        ...
        unit_check_general(1, nnz, hx_val_gold.data(), hx_val.data());
    }
