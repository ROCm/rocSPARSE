...
int main(int argc, char *argv[])
{
    ...
    rocsparse_dcsr2hyb(handle, m, n,
                       descrA, dAval, dAptr, dAcol,
                       hybA, 0, rocsparse_hyb_partition_max);
    ...
    for (int i=0; i<10; ++i)
    {
        ...
        rocsparse_dhybmv(handle, rocsparse_operation_none,
                         &halpha, descrA, hybA,
                         dx, &hbeta, dy);
    }
    ...
    double bandwidth = static_cast<double>(sizeof(double)*(2*m+nnz)
                                          +sizeof(rocsparse_int)*(nnz))/time/1e6;
